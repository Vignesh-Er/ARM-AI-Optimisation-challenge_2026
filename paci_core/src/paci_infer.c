...
// input and output share one scale. The actual trained model's
...
// ASSUMPTION: Tier-1 (INT4) is implemented in a follow-up commit once
// tools/export_cmsisnn.py's INT4 re-quantization path (Task 2.3, the
// fallback branch confirmed by tools/probe_int4.py) lands tier1_weights.h
// — deferred rather than stubbed out under a fake success path, since a
// fake PACI_OK here would be indistinguishable from a real Tier-1 result to
// any caller. Until then this always reports PACI_E_QUANT (missing export),
// the same "not built yet" signal PACI_HAVE_TIER2's #else branch uses.
#if !defined(PACI_HAVE_TIER1) || !PACI_HAVE_TIER1
